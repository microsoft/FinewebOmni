...
class MediaC4QualityFilter(BaseFilter):
    ...
    def filter(self, doc: Document) -> bool | tuple[bool, str]:
        ...
        assert self.split_paragraph, "Only supports split_paragraph=True now"
        ...
        media_filtered = []
        for media_id in range(len(doc.media)):
            media = doc.media[media_id]
            if media.type >= 0: # media
                media_filtered.append(media)
                continue
            kept_lines_local = []
            lines = media.alt.splitlines()
            ...
            for line in lines:
                ...
                kept_lines_local.append(line)
                self.stat_update("line-kept")
        
            media.alt = "\n".join(kept_lines_local).strip()
            if media.alt: # valid text
                media_filtered.append(media)
            
        if num_sentences < self.min_num_sentences:
            return False, "too_few_sentences"

        doc.media = media_filtered
